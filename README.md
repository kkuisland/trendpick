...
**클라우드 루틴 「케이트렌드 일일 발행·검증·배포」** 하나가 매일 **KST 07:07**(UTC `7 22 * * *`)에 돕니다. 깃허브 액션은 쓰지 않습니다 — 관리 화면은 https://claude.ai/code/routines 입니다.

한 회차 안에서 작성 → 검증 → 배포까지 끝냅니다.

| 단계 | 하는 일 | 실패하면 |
|---|---|---|
| 수집·기획 | `collect.mjs` → `plan.mjs` | 샌드박스에서 구글 피드가 403 으로 막히는 일이 잦다. 이벤트 주제로 넘어가 계속 진행 |
| 작성 | 하루 1편. 웹 검색으로 사실 확인 후 집필 | 쓸 주제가 없으면 **안 쓰는 것이 정상** |
| 검증 | `build.mjs` — 죽은 내부 링크·빌드 오류 | 죽은 링크가 남으면 **배포하지 않는다** |
| 배포 | 푸시 → Railway 자동 배포 → 라이브 200 확인 | 200 이 안 나오면 보고에 명시 |
| 색인 | `indexnow.mjs` | — |

언어는 자동으로 갈립니다: `content/en/posts/` 의 최신 글이 **4일 이상 지났으면 영문**, 아니면 한국어. 영문은 번역이 아니라 해외 독자 기준으로 새로 씁니다.

**왜 하루 1편인가.** 구글은 검수 없는 대량 AI 콘텐츠를 스팸으로 강등하며 초기 사이트일수록 위험이 큽니다. 하루 1편이라도 1년이면 300편 넘게 쌓입니다. 카테고리가 많다고 글 수를 늘리는 건 역효과입니다.

### 3중 안전장치

1. **일일/회당 한도** — `maxPostsPerDay`(1), `maxPostsPerRun`(1). 오늘 날짜의 글 파일을 직접 세므로 사람이 직접 글을 써도 총량을 넘지 않습니다.
2. **중복 방지** — 기존 글과 대조해 이미 다룬 주제·이벤트는 후보에서 제외합니다.
3. **점수 하한** — `minScore`(55) 미달이면 **아무것도 쓰지 않습니다.** "쓸 게 없으면 안 쓴다"가 정상 동작입니다.

### 민감 주제 자동 제외

재난·인명 피해, 범죄·수사·재판, 부고, 의료, 국내 정치, 투자 종목, 개인 사생활·논란 키워드는 **자동 생성에서 제외**됩니다 (`scripts/plan.mjs`의 `HARD_PATTERNS`/`SOFT_PATTERNS`). 기획 리포트에는 🚫 표시와 함께 계속 보이므로, 필요하면 사람이 직접 사실 확인 후 작성하면 됩니다.

제도·법률, 실존 인물, 의료, 금융, 통계, 시험·입시, 예매 일정이 걸린 글은 자동 발행되지 않고 `draft: true` 로 묶입니다. `node scripts/review-queue.mjs` 로 대기 목록을 봅니다.

### 검증에 무엇이 걸리는가

`node scripts/build.mjs` 의 `⚠️ SEO 점검` 에서 아래를 잡습니다.

- **죽은 내부 링크** — 글끼리 링크하는데 대상이 검수 보류(`draft: true`)면 빌드에서 빠져 404 가 됩니다. 눈으로 잡기 어려워 빌드가 대신 봅니다 (`scripts/lib/linkcheck.mjs`)
- `title`/`description` 누락, 제휴 링크 설정 오류, 분석 도구 설정 오류

### 수동으로 돌리려면

로컬에서 API 크레딧을 써서 도는 경로도 남아 있습니다 (`npm run auto`). 클라우드 루틴은 구독으로 돌기 때문에 평소에는 쓰지 않습니다.
...
모든 플랫폼에서 홈 화면에 설치해 앱처럼 열 수 있고, **아직 설치하지 않은 사람에게만** 하단에 설치 권장 배너가 뜹니다.

| 플랫폼 | 설치 여부를 아는 방법 | 배너 |
|---|---|---|
| 안드로이드·데스크탑 크롬/엣지 | `beforeinstallprompt` 는 미설치 상태에서만 발생 | 이벤트가 올 때만 표시, 버튼이 실제 설치창을 띄움 |
| iOS 사파리 | 설치 API 없음 — 홈 화면에서 열기 전에는 알 수 없음 | 2회차 방문부터 「공유 → 홈 화면에 추가」 안내 |
| 앱 안에서 열었을 때 | `display-mode: standalone` / `navigator.standalone` | 표시하지 않음 (설치 완료로 기록) |

- 첫 방문에는 띄우지 않습니다. 검색으로 처음 들어온 사람을 막아서면 이탈만 늘기 때문입니다
- 「나중에」를 누르면 30일, iOS 안내를 닫으면 90일 동안 다시 뜨지 않습니다 (`localStorage`의 `ktrend.pwa`)
- 서비스워커(`dist/sw.js`)는 **HTML을 항상 네트워크 우선**으로 가져옵니다. 하루 여러 번 글이 올라오는 사이트라 캐시가 본문을 덮으면 안 되기 때문이며, 캐시는 오프라인일 때만 쓰입니다
- 아이콘을 바꾸려면 `public/assets/favicon.svg` 를 고친 뒤 `node scripts/tools/make-icons.mjs` 로 PNG를 다시 만들어 커밋합니다 (빌드에 포함되지 않는 일회성 도구)
